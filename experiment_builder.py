...
class ExperimentBuilder(object):
    def __init__(self, args, data, model, device):
        ...
        self.model = model
        self.saved_models_filepath, self.logs_filepath, self.samples_filepath = build_experiment_folder(args=args, experiment_name=self.args.experiment_name)
        ...
        self.ID_coff_create = True

        if self.args.continue_from_epoch == 'from_scratch':  # 从 0 开始
            self.create_summary_csv = True
            ...
            if os.path.exists(checkpoint):
                self.state = self.model.load_model(model_save_dir=self.saved_models_filepath, model_name="train_model", model_idx='latest')
            ...
            self.state = self.model.load_model(model_save_dir=self.saved_models_filepath, model_name="train_model", model_idx=self.args.continue_from_epoch)
            self.start_epoch = int(self.state['current_iter'] / self.args.total_iter_per_epoch)
    ...
    def train_iteration(self, train_sample, sample_idx, epoch_idx, total_losses, current_iter, pbar_train):
        ...
        if sample_idx == 0:
            print("shape of data", x_support_set.shape, x_target_set.shape, y_support_set.shape, y_target_set.shape)
    ...
    def save_models(self, model, epoch, state):
        ...
        """
        model.save_model(model_save_dir=os.path.join(self.saved_models_filepath, "train_model_{}".format(int(epoch))), state=state)

        model.save_model(model_save_dir=os.path.join(self.saved_models_filepath, "train_model_latest"), state=state)
    ...
    def pack_and_save_metrics(self, start_time, create_summary_csv, train_losses, val_losses, state):
        ...
        if create_summary_csv:
            self.summary_statistics_filepath = save_statistics(self.logs_filepath, list(epoch_summary_losses.keys()), create=True)
            self.create_summary_csv = False

        start_time = time.time()
        print("epoch {} -> {}".format(epoch_summary_losses["epoch"], epoch_summary_string))

        self.summary_statistics_filepath = save_statistics(self.logs_filepath, list(epoch_summary_losses.values()))
    ...
    def evaluated_test_set_using_the_best_models(self, top_n_models):
        ...
        # ================================== 记录最佳文件
        with open(self.args.experiment_name + '/bestepoch.txt', 'w') as f:
            f.write(str(sorted_idx))
            f.write('\n')
            f.write(str(sorted_val_acc))
            f.close()
        ...
        for idx, model_idx in enumerate(top_n_idx):
            self.state = self.model.load_model(model_save_dir=self.saved_models_filepath, model_name="train_model", model_idx=model_idx + 1)
            with tqdm.tqdm(total=int(self.args.num_evaluation_tasks / self.args.batch_size)) as pbar_test:
                for sample_idx, test_sample in enumerate(self.data.get_test_batches(total_batches=int(self.args.num_evaluation_tasks / self.args.batch_size), augment_images=False)):
        ...
        _ = save_statistics(self.logs_filepath, list(test_losses.keys()), create=True, filename="test_summary.csv")

        summary_statistics_filepath = save_statistics(self.logs_filepath, list(test_losses.values()), create=False, filename="test_summary.csv")
        print(test_losses)
    ...
    def run_experiment(self):
        ...
        with tqdm.tqdm(initial=self.state['current_iter'], total=int(self.args.total_iter_per_epoch * self.args.total_epochs)) as pbar_train:
            while (self.state['current_iter'] < (self.args.total_epochs * self.args.total_iter_per_epoch)) and (self.args.evaluate_on_test_set_only == False):
                for train_sample_idx, train_sample in enumerate(self.data.get_train_batches(total_batches=int(self.args.total_iter_per_epoch * self.args.total_epochs) - self.state['current_iter'],
                    ...
                    if self.state['current_iter'] % self.args.total_iter_per_epoch == 0:
                        # ========================= 记录每次更新后的PID系数 ============================================
                        PID_coff_file = "{}/{}".format(self.args.experiment_name, 'PID-coff-lr.csv')

                        if self.ID_coff_create:
                            # --------------------------------- 第一次写入文件，需要创建文件
                            line_to_add = []
                            for i in range(10):
                                line_to_add.append('gen-I-' + str(i))

                            for j in range(10):
                                line_to_add.append('gen-D-' + str(j))

                            # ---------------------------------------- 写表头
                            with open(PID_coff_file, 'w', newline='') as f:
                                writer = csv.writer(f)
                                writer.writerow(line_to_add)

                            # ----------------------------------- 写数据
                            line_to_add = []
                            line_to_add.extend([_ for _ in self.model.generated_I.data.cpu().numpy()])
                            line_to_add.extend([_ for _ in self.model.generated_D.data.cpu().numpy()])

                            with open(PID_coff_file, 'a', newline='') as f:
                                writer = csv.writer(f)
                                writer.writerow(line_to_add)

                            self.ID_coff_create = False
                        else:  # 后面直接写数据就可以
                            # ----------------------------------- 写数据
                            line_to_add = []
                            line_to_add.extend([_ for _ in self.model.generated_I.data.cpu().numpy()])
                        ...
                        with tqdm.tqdm(total=int(self.args.num_evaluation_tasks / self.args.batch_size)) as pbar_val:
                            for _, val_sample in enumerate(self.data.get_val_batches(total_batches=int(self.args.num_evaluation_tasks / self.args.batch_size), augment_images=False)):
                                val_losses, total_losses = self.evaluation_iteration(val_sample=val_sample, total_losses=total_losses, pbar_val=pbar_val, phase='val')
                        ...
                        self.epoch += 1
                        self.state = self.merge_two_dicts(first_dict=self.merge_two_dicts(first_dict=self.state, second_dict=train_losses), second_dict=val_losses)
                        ...
                        if self.epochs_done_in_this_run >= self.total_epochs_before_pause:
                            print("train_seed {}, val_seed: {}, at pause time".format(self.data.dataset.seed["train"], self.data.dataset.seed["val"]))
                            sys.exit()
