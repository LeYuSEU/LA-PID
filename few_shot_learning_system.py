...
class MAMLFewShotClassifier(nn.Module):
    def __init__(self, im_shape, device, args):
        ...
        if self.args.backbone == 'ResNet12':
            self.classifier = ResNet12(im_shape=self.im_shape, num_output_classes=self.args.num_classes_per_set, args=args, device=device, meta_classifier=True).to(device=self.device)
        else:
            self.classifier = VGGReLUNormNetwork(im_shape=self.im_shape, num_output_classes=self.args.num_classes_per_set, args=args, device=device, meta_classifier=True).to(device=self.device)
        ...
        self.inner_loop_optimizer = LSLRGradientDescentLearningRule(device=device,
                                                                    ...
                                                                    use_learnable_weight_decay=self.args.LAPID,
                                                                    use_learnable_learning_rates=self.args.LAPID,
                                                                    LAPID=self.args.LAPID,
        ...
        self.generated_I, self.generated_D, self.generated_P = None, None, None

        print("Inner Loop parameters") # 内环学习的参数就是 alpha 和 beta
        ...
        self.regularizer = nn.Sequential(
            nn.Linear(num_layers * 3, input_dim),
    ...
    def get_per_step_loss_importance_vector(self):
        ...
        curr_value = np.minimum(loss_weights[-1] + (self.current_epoch * (self.args.number_of_training_steps_per_iter - 1) * decay_rate),
    ...
    def apply_inner_loop_update(self, loss, names_weights_copy, 
                                # generated_P_params,
                                generated_I_params, generated_D_params,
        ...
        names_weights_copy = self.inner_loop_optimizer.update_params(names_weights_dict=names_weights_copy,
                                                                    ...
                                                                    #  generated_P_params=generated_P_params,
                                                                     generated_I_params=generated_I_params,
                                                                     generated_D_params=generated_D_params,
        ...
        for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in \
            ...
            for num_step in range(num_steps):
                ...
                if self.args.LAPID:
                    ...
                    for k, v in names_weights_copy.items():  # [1, 48, 3, 3, 3] ---> 取均值来代替，拿出网络的参数 均值
                        # if 'bias' in k:
                        #     continue
                        per_step_task_embedding.append(v.mean())
                        per_step_task_embedding.append(v.std())

                    for i in range(len(support_loss_grad)):  # 拿出梯度, 也是对应的均值
                    ...
                    # self.generated_P, self.generated_I, self.generated_D = torch.split(generated_params, split_size_or_sections=num_layers)
                    self.generated_I, self.generated_D = torch.split(generated_params, split_size_or_sections=num_layers)
                    g = 0
                    for key in names_weights_copy.keys():
                        # generated_P_params[key] = self.generated_P[g]
                        generated_I_params[key] = self.generated_I[g]
                        generated_D_params[key] = self.generated_D[g]
                        g += 1
                ...
                if use_multi_step_loss_optimization and training_phase and epoch < self.args.multi_step_loss_num_epochs:
                    ...
                    # print("num_step = ", num_step, " self.args.number_of_training_steps_per_iter = ", self.args.number_of_training_steps_per_iter)
    ...
    def net_forward(self, x, y, weights, backup_running_statistics, training, num_step):
        ...
        loss = F.cross_entropy(input=preds, target=y) #  + self.generated_I + self.generated_D
    ...
    def meta_update(self, loss):
        ...
        # loss = loss +  (torch.pow(self.generated_I, 2).sum() + torch.pow(self.generated_D, 2).sum())  / len(self.generated_D)
        loss.backward()
        self.optimizer.step()
