...
def augment_image(image, k, channels, augment_bool, args, dataset_name):
    transform_train, transform_evaluation = get_transforms_for_dataset(dataset_name=dataset_name, args=args, k=k)
...
def get_transforms_for_dataset(dataset_name, args, k):
    if "cifar10" in dataset_name or "cifar100" in dataset_name or "FC100" in dataset_name:
        transform_train = [transforms.ToTensor(), transforms.Normalize((0.5071, 0.4847, 0.4408), (0.2675, 0.2565, 0.2761))]
        transform_evaluate = [transforms.ToTensor(), transforms.Normalize((0.5071, 0.4847, 0.4408), (0.2675, 0.2565, 0.2761))]
        ...
        transform_train = [transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])]
        transform_evaluate = [transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])]
...
class FewShotLearningDatasetParallel(Dataset):
    ...
    def load_dataset(self):
        ...
        if self.args.sets_are_pre_split:
            ...
            x_train_id = int(self.train_val_test_split[0] * total_label_types)
            x_val_id = int(np.sum(self.train_val_test_split[:2]) * total_label_types)
            x_test_id = int(total_label_types)
            ...
            x_train = {class_key: data_image_paths[class_key] for class_key in x_train_classes}
            x_val = {class_key: data_image_paths[class_key] for class_key in x_val_classes}
            x_test = {class_key: data_image_paths[class_key] for class_key in x_test_classes}
    ...
    def load_parallel_batch(self, inputs):
        ...
        if self.data_loaded_in_memory:
            ...
            image_batch = [self.load_image(image_path=image_path, channels=self.image_channel) for image_path in batch_image_paths]
            # load_pbar.update(1)
    ...
    def get_set(self, dataset_name, seed, augment_images=False):
        ...
        for class_entry in selected_classes:
            choose_samples_list = rng.choice(self.dataset_size_dict[dataset_name][class_entry], size=self.num_samples_per_class + self.num_target_samples, replace=False)
            class_image_samples = []
            class_labels = []
    ...
    def __getitem__(self, idx):
        support_set_images, target_set_image, support_set_labels, target_set_label, seed = \
            self.get_set(self.current_set_name, seed=self.seed[self.current_set_name] + idx, augment_images=self.augment_images)
